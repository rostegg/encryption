...
public class Blowfish {
    ...
    private boolean nonStandardMethod;
    ...
    public String Encrypt_CBC(String pt)
    {
        if (!IVSet)
            SetRandomIV();
        return ByteToHex(InitVector) + ByteToHex(Encrypt_CBC(pt.getBytes(StandardCharsets.US_ASCII)));
    }
    /*
    public String Decrypt_CBC(String ct)
    {
    	InitVector = HexToByte(ct.substring(0, 16));
	...
	*/
    ...
    {
        return ByteToHex(Encrypt_ECB(pt.getBytes(StandardCharsets.US_ASCII)));
    }
    /*
    ...
    private void SetupKey(byte[] cipherKey)
    {
        ...
        for (int i = 0; i < 18; i++)
        {
            long d = (long)(((key[j % cipherKey.length] * 256 + key[(j + 1) % cipherKey.length]) * 256 + key[(j + 2) % cipherKey.length]) * 256 + key[(j + 3) % cipherKey.length]);
            bf_P[i] ^= d;
    ...
    private byte[] Crypt_ECB(byte[] text, boolean decrypt)
    {
        ...
        {
        	System.arraycopy(plainText, i, block, 0, 8);
            if (decrypt)
            {
                BlockDecrypt(block);
            }
            ...
            System.arraycopy(block, 0, plainText, i, 8);
        }
        return plainText;
    }
    ...
    private byte[] Crypt_CBC(byte[] text, boolean decrypt)
    {
        if (!IVSet)
        {
        }
        ...
        byte[] iv = new byte[8];
        System.arraycopy(InitVector, 0, iv, 0, 8);
        if (!decrypt)
        {
            for (int i = 0; i < plainText.length; i += 8)
            {
        ...
        else
        {
            for (int i = 0; i < plainText.length; i += 8)
            {
            	...
            	System.arraycopy(block, 0, preblock, 0, 8);
                BlockDecrypt(block);
                XorBlock(block, iv);
                System.arraycopy(preblock, 0, iv, 0, 8);
    ...
    private void XorBlock(byte[] block, byte[] iv)
    {
    ...
    private void SetBlock(byte[] block)
    {
        ...
        {
            xr_par = getIntValue(block1,FORMAT_UINT32, 0);
            xl_par = getIntValue(block2,FORMAT_UINT32, 0);
        }
        else
        {
            reverse(block1);
            reverse(block2);
            xl_par = getIntValue(block1,FORMAT_UINT32, 0);
            xr_par = getIntValue(block2,FORMAT_UINT32, 0);
        }
    }
    ...
    private void GetBlock(byte[] block)
    {
        ...
        if (nonStandardMethod)
        {
            block1 = GetBytes(xr_par);
            block2 = GetBytes(xl_par);
        }
        else
        {
            block1 = GetBytes(xl_par);
            block2 = GetBytes(xr_par);

            reverse(block1);
            reverse(block2);
        }
        System.arraycopy(block1, 0, block, 0, 4);
        System.arraycopy(block2, 0, block, 4, 4);
    }
    ...
    public static byte[] GetBytes(long xr_par2)
    {
        ByteBuffer buffer = ByteBuffer.allocate(4).order(ByteOrder.nativeOrder());
        buffer.putLong(xr_par2);
        return buffer.array();
    }
    ...
    private long round(long a, long b, long n)
    {
    	long x1 = (bf_s0[wordByte0(b)] + bf_s1[wordByte1(b)]) ^ bf_s2[wordByte2(b)];
    	long x2 = x1 + bf_s3[this.wordByte3(b)];
    	long x3 = x2 ^ bf_P[(int) n];
        return x3 ^ a;
    }
    ...
    private byte wordByte0(long w)
    {
        return (byte)(w & 0xFF);
    }

    private byte wordByte1(long w)
    {
        return (byte)((w >> 8) & 0xFF);
    }

    private byte wordByte2(long w)
    {
        return (byte)((w>> 16) & 0xFF);
    }

    private byte wordByte3(long w)
    {
    ...
    private byte[] HexToByte(String hex)
    {
        ...
        {
            byte a = GetHex(hex.toCharArray()[i]);
            byte b = GetHex(hex.toCharArray()[i + 1]);
            r[i / 2] = (byte)(a * 16 + b);
        }
        return r;
    }
