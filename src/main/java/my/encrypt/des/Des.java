...
public class Des {
	...
	private static byte[] encrypt64Bloc(byte[] block,byte[][] subkeys, boolean isDecrypt) {
		byte[] tmp = new byte[block.length];
		byte[] R = new byte[block.length / 2];
		byte[] L = new byte[block.length / 2];

		tmp = permutFunc(block, ip);
	...
	public static byte[] tripleEncrypt(byte[] data,byte[][] keys)
	{
		int lenght=0;
		byte[] padding = new byte[1];
		int i;
		...
		for (i = 0; i < data.length + lenght; i++) {
			if (i > 0 && i % 8 == 0) {
				block = encrypt64Bloc(block,K, false);						
				block = encrypt64Bloc(block,K1, true);		
				block = encrypt64Bloc(block,K2, false);
				System.arraycopy(block, 0, tmp, i - 8, block.length);
			}
			if (i < data.length)
				block[i % 8] = data[i];
	...
	public static byte[] tripleDecrypt(byte[] data,byte[][] keys)
	{
		int i;
		...
		for (i = 0; i < data.length; i++) {
			if (i > 0 && i % 8 == 0) {
				block = encrypt64Bloc(block,K2, true);
				block = encrypt64Bloc(block,K1, false);
				block = encrypt64Bloc(block,K, true);
				System.arraycopy(block, 0, tmp, i - 8, block.length);
			}
			if (i < data.length)
				block[i % 8] = data[i];
